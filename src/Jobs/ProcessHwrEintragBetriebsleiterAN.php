...
class ProcessHwrEintragBetriebsleiterAN implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $typ = Typ::where('form_identifier', 'hwreintrag-betriebsleiter-an')->first();
        if ($this->uuid) {
            $webhook = $typ->webhooks()->latest()->wherePivot('formwerk_uuid', $this->uuid)->first()->pivot;
            Log::info('ProcessHwrEintragBetriebsleiterAN - Found webhhok with uuid: '.$this->uuid);
        } else {
            $webhook = $typ->webhooks()->latest()->wherePivot('identifier', $this->identifier)->first()->pivot;
            Log::info('ProcessHwrEintragBetriebsleiterAN - Found webhhok with identifier: '.$this->identifier);
        }
        ...
        Log::info('ProcessHwrEintragBetriebsleiterAN abgeschlossen', [
